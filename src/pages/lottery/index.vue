<template>
  <view :class="style.container">
    <view :class="style.content">
      ...
      <view :class="style.subtitle">{{ rules }}</view>
      <view :class="style.box">
        ...
        <view :class="style.item" @tap="handleClick">
          <view :class="style.itemborder">
            <view :class="style.itemcontent">
              <view
                :class="[style.contentbtn, disable ? style.btndisable : '']"
              >
        ...
        <view v-if="showPrize" :class="style.result" @tap="handlePrizeClick">
          <view :class="style.resulttext"> 恭喜获得奖品 </view>
          <view
            :style="{ backgroundImage: `url(${prize.image})` }"
            :class="[style.resultimage]"
          />
          <view :class="style.resulttext">{{ prize.text }}</view>
        </view>
      </view>
    </view>
  </view>
...
export default {
  ...
  data() {
    return {
      ...
      num: 1, //每天的抽奖次数
      total: 5, //全部的抽奖机会
      duration: ["2021-4-1", "2021-4-5"], //活动时间
      disable: true,
    };
  },
  computed: {
  ...
  methods: {
    ...
    controlRandom() {
      ...
      for (let index = 0; index < this.itemIDList.length; index++) {
        sum = this.itemIDList.slice(0, index + 1).reduce((sum, item) => {
